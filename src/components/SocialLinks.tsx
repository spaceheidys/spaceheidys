  ...
  return (
    ...
    >
      {socials.map((s) => (
        ...
        >
          <img src={s.icon} alt={s.label} <img src={s.icon} alt={s.label} style={{ height: 29 }} className="w-auto opacity-60 hover:opacity-100 transition-opacity" /> className="w-auto opacity-60 hover:opacity-100 transition-opacity" />
        </a>
      ))}
    </motion.div>
  );
