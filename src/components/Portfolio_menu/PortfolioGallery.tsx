    ...
    >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          ref={gridRef}
          className={`w-full grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-2 px-2 sm:px-2 py-0.5 sm:p-1 content-center`}
        ...
        >
          {pageItems.map((item, i) => (
            <motion.div
              key={item.id}
              className="aspect-[4/3] bg-white/5 border border-white/10 rounded-md flex items-center justify-center cursor-pointer hover:bg-white/10 hover:border-white/20 transition-colors duration-300 overflow-hidden"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
      ...
      <AnimatePresence>
        {zoomedImg && (
          <motion.div
            className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 cursor-zoom-out overflow-auto"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
